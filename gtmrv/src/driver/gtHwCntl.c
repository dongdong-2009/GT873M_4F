	...
	if(IS_IN_DEV_GROUP(dev,DEV_8PORT_SWITCH))
	{
		switch(accessType)
		{
			case PHY_ACCESS:
					...
					{
                        /*wugang add 2011.6.29 GIS2109, which use mrv3018 phy chip.*/
                        if ((IS_IN_DEV_GROUP(dev, DEV_88E6083))&&(9 == portNum))
                        {
                            smiAddr = 0x1c;
                        }
					else if ((IS_IN_DEV_GROUP(dev, DEV_88E6097_FAMILY))&&(9 == portNum))
		                        {
		                            smiAddr = 0x9;
		                        }			
                        else
