...
/* CHIP       : CS8030                                                       */
/*                                                                           */
void oam_orgspec_handle(cs_port_id_t port,cs_uint8 *frame, cs_uint32 len)
/*                                                                           */
...
/*****************************************************************************/
{
    ...
    if(port == CS_PON_PORT_ID){
        ...
        oam_orgspec_handle(port,frame_ptr, len);
    }
...
#ifdef HAVE_ZTE_OAM
    oam_zte_pon_tx_ctrl *pData = (oam_zte_pon_tx_ctrl *)pdu->data;
#endif
...
#ifdef HAVE_ZTE_OAM
        if((pData->hdr.branch == OAM_CTC_ATTRIB_BRANCH)
                && (ntohs(pData->hdr.leaf) == 0x801a )
                ){
            zte_oam_tx_laser_ctrl_set_adapt(pData->match_mac, pData->mac, pData->action, ntohs(pData->duration));
        }
        else
#endif
