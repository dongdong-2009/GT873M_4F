    ...
    mc_group_record_list_t *record)
{
    ...
    if(IGMP_V1_REPORT == record->entry.pdu_type ||
       ...
       MLD_V1_REPORT == record->entry.pdu_type) {
           ...
           if(update_time < port_grp->entry.update_time + 100) {
...
cs_status mc_grp_report_vchange_handler(mc_node_t * node, mc_object_t *msg)
{
	...
	if((msg->ip_type == MC_IPV4) && (msg->pdu_type == IGMP_V3_REPORT) && mcV3ToV2IsEnable())
	{
		...
		cs_lst_scan(record_list, record, mc_group_record_list_t*){
			if(record->entry.pdu_type != IGMP_V3_REPORT)
				continue;


			pdu = (mc_pdu_t*)(msg->frame+msg->ether_head_len+msg->ip_head_len);

			MC_LOG(IROS_LOG_LEVEL_INF,"v3 record: type = %d, src num=%d\r\n", record->entry.record_type,
					record->entry.src_list.count);
			if((record->entry.record_type == MODE_IS_INCLUDE
					|| record->entry.record_type == CHANGE_TO_INCLUDE_MODE) &&
			...
			pdu->grp_key.ipv4_grp = htonl(record->entry.mc_grp.addr.ipv4);
			pdu->checksum = 0;

			pdu->checksum = inet_chksum(pdu, sizeof(mc_pdu_t));

//			modified dest mac and ip address
			dip = (pdu->type == IGMP_V2_REPORT)?record->entry.mc_grp.addr.ipv4:0xe0000002;

			ip.addr.ipv4 = dip;
			ip.type = MC_IPV4;
			mc_ip_to_mac(&ip, ether_hdr->dest);

			ipv4_hdr->dst_ip = htonl(dip);
			ipv4_hdr->check = 0;
...
{   
    ...
    if(mc_grp_record_parse(node,msg) == CS_E_OK) {
        ...
        if(CS_E_OK == mc_grp_record_list_proc(node, port, &msg->record_list)) {
            if(MC_PROXY != node->mode && (mc_grp_report_vchange_handler(node, msg) != CS_E_OK)) {
                if(node->up_port) {
