...
cs_status mc_pkt_object_build(mc_node_t *node, cs_pkt_t *pkt, mc_object_t *message)
{
    ...
    /* checksum filter */
    if(MC_IPV4 == message->ip_type) {
        cs_uint16 pkt_cs = message->pdu->checksum;
        cs_uint32 calc_cs = 0;
        cs_uint16 real_cs = 0;
        cs_uint16 pdu_len = message->pdu_len;
        cs_uint16 *pdu = (cs_uint16 *)pBuf;

        message->pdu->checksum = 0;

        while (pdu_len > 1) {
            calc_cs += *(pdu++);
            pdu_len -= sizeof(cs_uint16);
        }
        
        if(pdu_len) {
            calc_cs += *((cs_uint8*)pdu);
        }
        
        message->pdu->checksum = pkt_cs;

        calc_cs = (calc_cs >> 16) + (calc_cs & 0xffff);
        calc_cs += (calc_cs >> 16);

        real_cs = (cs_uint16)~calc_cs;
        
        if(real_cs != pkt_cs) {
            MC_LOG(IROS_LOG_LEVEL_INF, "checksum 0x%04x error!(0x%04x)\n", pkt_cs, real_cs);
            return CS_E_ERROR;
        }
    }
    else {  /* IPV6 */
