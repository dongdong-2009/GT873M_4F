 ...
 * interrupt.c
 *
 * $Id: interrupt.c,v 1.1.2.5.6.1.4.1.2.1.12.7.2.7.2.1.2.19 2011/11/09 10:08:03 xyu Exp $
 */
/****************************************************************
OVERVIEW
...
extern void sdl_glb_int_process();

static cs_int32 cs_intr_handler_glb(cs_int32 vector , void *data)
{
    sdl_glb_int_process();
    return CS_E_OK;
}
...
void onu_interrupt_init()
{
    ...
    cs_intr_reg(ONU_INT_FLAGBIT_GLB   , "glb_intr",  cs_intr_handler_glb    , NULL, INTR_THREAD_CB);
...
#ifdef HAVE_TERMINAL_SERVER
    cs_plat_uart_intr_handler_set(context,0,0,1,(cs_uart_intr_handler_t)ts_uart_intr_handler,0);
    cs_plat_uart_intr_handler_set(context,0,0,2,(cs_uart_intr_handler_t)ts_uart_intr_handler,0);
    cs_plat_uart_intr_handler_set(context,0,0,3,(cs_uart_intr_handler_t)ts_uart_intr_handler,0);
    cs_plat_uart_intr_handler_set(context,0,0,4,(cs_uart_intr_handler_t)ts_uart_intr_handler,0);
#endif
#ifdef HAVE_CPULOAD_PER_THREAD
    hw_cpuload_timer_id = cs_hw_circle_timer_add(1000, cs_cpuload_sampling, NULL);
#endif
}
cs_uint32 uart_ver[5] = {6,7,29,30,31};
void onu_interrupt_disable_w(cs_uint32 uart_port)
{
	cs_int32 vector;
	vector = uart_ver[uart_port];
	//cyg_interrupt_detach(intr_list[vector].handle);
	cyg_interrupt_mask(vector);
}
void onu_interrupt_enable_w(cs_uint32 uart_port)
{
	cs_int32 vector;
	vector = uart_ver[uart_port];
	//cyg_interrupt_attach(intr_list[vector].handle);
...
extern cs_int32 text_uart_intr_handler(cs_uint8 uart_id,void *data);
cs_uint32 text_uart_read_enable()
{
	cs_status ret;
	ret = CS_E_OK;
	cs_callback_context_t       context;
	ret = cs_plat_uart_intr_handler_set(context,0,0,1,(cs_uart_intr_handler_t)text_uart_intr_handler,0);
	if(ret != CS_E_OK)
		{
			cs_printf("set uart 1 intr handler reg fail\n");
			return 0;
		}
    ret = cs_plat_uart_intr_handler_set(context,0,0,2,(cs_uart_intr_handler_t)text_uart_intr_handler,0);
	if(ret != CS_E_OK)
		{
			cs_printf("set uart 1 intr handler reg fail\n");
			return 0;
		}
    ret = cs_plat_uart_intr_handler_set(context,0,0,3,(cs_uart_intr_handler_t)text_uart_intr_handler,0);
	if(ret != CS_E_OK)
	{
		cs_printf("set uart 1 intr handler reg fail\n");
		return 0;
	}
    ret = cs_plat_uart_intr_handler_set(context,0,0,4,(cs_uart_intr_handler_t)text_uart_intr_handler,0);
	if(ret != CS_E_OK)
	{
		cs_printf("set uart 1 intr handler reg fail\n");
		return 0;
...
cs_uint32 text_uart_read_disable()
{
	cs_status ret;
	 ret = CS_E_OK;
	cs_callback_context_t       context;
	ret = cs_plat_uart_intr_handler_set(context,0,0,1,(cs_uart_intr_handler_t)ts_uart_intr_handler,0);
	if(ret != CS_E_OK)
		{
			cs_printf("set uart 1 intr handler reg fail\n");
			return 0;
		}
    ret = cs_plat_uart_intr_handler_set(context,0,0,2,(cs_uart_intr_handler_t)ts_uart_intr_handler,0);
	if(ret != CS_E_OK)
		{
			cs_printf("set uart 1 intr handler reg fail\n");
			return 0;
		}
    ret = cs_plat_uart_intr_handler_set(context,0,0,3,(cs_uart_intr_handler_t)ts_uart_intr_handler,0);
	if(ret != CS_E_OK)
	{
		cs_printf("set uart 1 intr handler reg fail\n");
		return 0;
	}
    ret = cs_plat_uart_intr_handler_set(context,0,0,4,(cs_uart_intr_handler_t)ts_uart_intr_handler,0);
	if(ret != CS_E_OK)
	{
		cs_printf("set uart 1 intr handler reg fail\n");
		return 0;
