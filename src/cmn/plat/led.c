...
#ifdef HAVE_ZTE_OAM
cs_uint32 g_ups_led_gpio_id = 0;
cs_uint32 g_ups_led_tmr = 0;
cs_uint32 g_run_led_gpio_id = 0;
cs_uint32 g_run_led_tmr = 0;
cs_uint32 g_los_led_gpio_id = 0;
cs_uint32 g_los_led_tmr = 0;
cs_uint32 g_pon_port_led_gpio_id = 0;
cs_uint32 g_pon_port_led_tmr = 0;

cs_status onu_ups_led_init(cs_uint32 map)
{
    cs_uint32 i;
    for(i=0; i<ONU_GPIO_MAX_NUM; i++) {
        if(map & (1<<i) ) {
    ...
    onu_led_proc(ONU_LED_UPS, ONU_LED_TURN_OFF);
    return CS_E_OK;
}

cs_status onu_run_led_init(cs_uint32 map)
{
    cs_uint32 i;
    for(i=0; i<ONU_GPIO_MAX_NUM; i++) {
        if(map & (1<<i) ) {
    ...
    onu_led_proc(ONU_LED_RUN, ONU_LED_TWINKLE);
    return CS_E_OK;
}

cs_status onu_los_led_init(cs_uint32 map)
{
    cs_uint32 i;
    for(i=0; i<ONU_GPIO_MAX_NUM; i++) {
        if(map & (1<<i) ) {
    ...
    onu_led_proc(ONU_LED_LOS, ONU_LED_TURN_ON);
    return CS_E_OK;
}

cs_status onu_pon_port_led_init(cs_uint32 map)
{
    cs_uint32 i;
    for(i=0; i<ONU_GPIO_MAX_NUM; i++) {
        if(map & (1<<i) ) {
            g_pon_port_led_gpio_id = i;
            break;
        }
    }
    ...
    onu_led_proc(ONU_LED_PON, ONU_LED_TWINKLE_FAST);
    return CS_E_OK;
}
...
#ifdef HAVE_ZTE_OAM
    case ONU_LED_UPS:
        onu_single_led_proc(g_ups_led_gpio_id, action, &g_ups_led_tmr);
        break;
    case ONU_LED_RUN:
        onu_single_led_proc(g_run_led_gpio_id, action, &g_run_led_tmr);
        break;
    case ONU_LED_LOS:
        onu_single_led_proc(g_los_led_gpio_id, action, &g_los_led_tmr);
        break;
    case ONU_LED_PON:
        onu_single_led_proc(g_pon_port_led_gpio_id, action, &g_pon_port_led_tmr);
        break;
#endif
