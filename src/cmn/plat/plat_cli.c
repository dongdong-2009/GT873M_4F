...
#define UART
cs_uint32 PORT_UART;
extern cs_uint8 char_buf[1000];
extern void mon_write_char_uart(char c);
...
cs_uint8 rev_buf_two[100]= {0};
cs_uint8 rev_buf_thr[100]= {0};
cs_uint8 rev_buf_for[100]= {0};
cs_uint32 rev_uart_msg_one = 0;
cs_uint32 rev_uart_msg_two = 0;
    ...
    else
    {
        if(cmd_cmp(argv[1], "read"))
        {
              if(argc == 5)
              {
                    ...
                    cs_plat_i2c_read(context,0,0,iros_strtol(argv[2]),iros_strtol(argv[3]), iros_strtol(argv[4]),p_buf);
        ...
        else if(cmd_cmp(argv[1], "write"))
        {
              if(argc == 5)
              {
                    ...
                    cs_plat_i2c_write(context,0,0,iros_strtol(argv[2]),iros_strtol(argv[3]), 1,(unsigned char *)&data);

                    cs_plat_i2c_read(context,0,0,iros_strtol(argv[2]),iros_strtol(argv[3]),1,&data);
...
cs_uint32 UART_RATE[5];
extern void ts_save_config(cs_uint8 uart_id,cs_uint8 *buff, cs_int32 len);
extern void  uart_send(cs_uint8 uart_id,char * buffer, unsigned int len);
extern void ts_get_config(cs_uint8 uart_id,cs_uint8 *buff, cs_int32 len);
    ...
    else
    {
        if(cmd_cmp(argv[1], "cfg"))
        {
              if(argc == 8)
              {
					...
					//cs_printf("uart_rate:%d\n",uart_cfg.baud_rate);
					UART_RATE[uart_cfg.uart_id] = uart_cfg.baud_rate;
					#if 0
                    cs_plat_uart_disable(context,0,0,iros_strtol(argv[2]));
                    cs_plat_uart_baudrate_set(context,0,0,iros_strtol(argv[2]),iros_strtol(argv[3]));
                    cs_plat_uart_databits_set(context,0,0,iros_strtol(argv[2]),iros_strtol(argv[4]));
                    cs_plat_uart_parity_set(context,0,0,iros_strtol(argv[2]),iros_strtol(argv[5]));
                    cs_plat_uart_mode_set(context,0,0,iros_strtol(argv[2]),iros_strtol(argv[6]));
                    cs_plat_uart_duplex_set(context,0,0,iros_strtol(argv[2]),iros_strtol(argv[7]));
                    cs_plat_uart_enable(context,0,0,iros_strtol(argv[2]));
					cs_printf("set uart cfg success\n");
					#else
					...
					db_ts_cfg.data_bits = uart_cfg.data_bits;
					db_ts_cfg.parity = uart_cfg.parity;
					db_ts_cfg.mode = uart_cfg.mode;
					db_ts_cfg.duplex = uart_cfg.duplex;
					db_ts_cfg.baud_rate = uart_cfg.baud_rate;
					db_ts_cfg.uart_id = uart_cfg.uart_id;
					ts_save_config(uart_cfg.uart_id,(cs_uint8 *)&db_ts_cfg, sizeof(db_ts_cfg_t));
					cs_printf("set uart cfg success\n");
					#endif
		...
		else if(cmd_cmp(argv[1],"show"))
			{
        		...
        		ts_get_config(uart_cfg.uart_id,(cs_uint8 *)pTemp,sizeof(db_ts_cfg_t));
				#if 0
				cs_plat_uart_baudrate_get(context,0,0,uart_cfg.uart_id,&uart_cfg.baud_rate);
				cs_plat_uart_databits_get(context,0,0,uart_cfg.uart_id,&uart_cfg.data_bits);
				cs_plat_uart_parity_get(context,0,0,uart_cfg.uart_id,&uart_cfg.parity);
				cs_plat_uart_mode_get(context,0,0,uart_cfg.uart_id,&uart_cfg.mode);
				cs_plat_uart_duplex_set(context,0,0,uart_cfg.uart_id,uart_cfg.duplex);
				#endif
				cs_printf("======================UART CONFIG=======================\n");
				cs_printf("UART PORT: %d\n",uart_cfg.uart_id);
				cs_printf("UART BAUDRATE: %d\n",pTemp->baud_rate);
				cs_printf("UART DATABITS: %d\n",pTemp->data_bits);
				switch(pTemp->parity)
					{
						case 0:
							cs_printf("UART PARITY: None\n");
							break;
						case 1:
							cs_printf("UART PARITY: Odd\n");
							break;
						case 2:
							cs_printf("UART PARITY: Even\n");
							break;
							default:
								break;
					}
				cs_printf("UART MODE: %s\n",pTemp->mode>1?"Polling":"Interrupt");
				cs_printf("UART DUPLEX: %s\n",pTemp->duplex?"Half-duplex":"Full-duplex");
				cs_printf("========================================================\n");
			    iros_free(pTemp);
			}
        else if(cmd_cmp(argv[1], "test"))
        {
			...
			//cs_uint32 port = 0;
                if(argc == 3)
                {
					...
					#if 0
					if(cmd_cmp(argv[2], "send_frame"))
						{
							port = iros_strtol(argv[3]);
							sed = argv[4];
							uart_send(port,(char *)sed,strlen(sed));
						}
					else
						{
        	...
        	if(argc == 4)
        		{
        			unsigned char uart_id;
					uart_id = iros_strtol(argv[2]);
					if((!cmd_cmp(argv[3],"on"))&&(!cmd_cmp(argv[3],"off")))
						{
							cs_printf("input error\n");
							cs_printf("\tplease input <read> <uart_port><on/off>\n");
						}
...
static int mdio_cmd_proc(int argc, char **argv)
{
    ...
    else
    {
        if(cmd_cmp(argv[1], "read"))
        {
             if(argc == 4)
            {
                    cs_plat_mdio_speed_set(context,0,0,iros_strtol(argv[2]), 4000);
                    rc = cs_plat_mdio_read(context,0,0,iros_strtol(argv[2]), 
        ...
        else if(cmd_cmp(argv[1], "write"))
        {
            ...
            {
                    cs_plat_mdio_speed_set(context,0,0,iros_strtol(argv[2]), 4000);
                    rc = cs_plat_mdio_write(context,0,0,iros_strtol(argv[2]), iros_strtol(argv[3]), iros_strtol(argv[4]));
...
static int watchdog_cmd_proc(int argc, char **argv)
{
    ...
    else
    {
        ...
        {
            cs_plat_wdt_enable(context,0,0);
        }
        ...
        {
             cs_plat_wdt_interval_get(context,0,0,(cs_uint32 *)&value);
            cs_printf("watchdog timeout value is %d second\n",value);
        }
        else if(cmd_cmp(argv[1], "set"))
        {
           if(argc == 3)
               cs_plat_wdt_interval_set(context,0,0,iros_strtol(argv[2]));
           else
               cs_printf("per wdt set value- set watchdog timeout value(second)\n");
        }
        else
...
static void do_term_server_help(void)
{
    ...
    cs_printf("            --<max>: 1024~1400 for maximum playload length\n");
...
#ifdef HAVE_ZTE_OAM
extern cs_int32 gpio_reset_flag;
#endif
...
#ifdef HAVE_ZTE_OAM
    if(argc == 2 && cmd_cmp(argv[0],"wd") && cmd_cmp(argv[1],"disable"))
    {
        gpio_reset_flag = 1;
        return 1;
    }
