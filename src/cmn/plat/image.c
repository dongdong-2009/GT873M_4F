 ...
 * image.c
 *
 * $Id: image.c,v 1.1.4.7.6.1.2.3.2.1.2.2.8.8.4.1.2.8 2011/10/26 03:12:10 ljin Exp $
 */
...
cs_uint32 iros_image_commit(void)
{
	...
	#else
	temp_ptr=malloc(flash_dev.info.super_block.block_size);
	cs_printf("***malloc\n");
	#endif
	#if 0
	cs_printf("flash_dev.info.super_block.block_size :0x%x\n", flash_dev.info.super_block.block_size);
	#endif
    if(NULL == temp_ptr){
