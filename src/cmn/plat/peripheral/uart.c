...
static uart_channel_t uart_channels[MAX_UART_NUM] = {
        {IMST_UART0_BASE,ONU_INT_FLAGBIT_UART0, NULL},
        {IMST_UART1_BASE,ONU_INT_FLAGBIT_UART1, NULL},
        {IMST_UART2_BASE,ONU_INT_FLAGBIT_UART2, NULL},
        {IMST_UART3_BASE,ONU_INT_FLAGBIT_UART3, NULL},
        {IMST_UART4_BASE,ONU_INT_FLAGBIT_UART4, NULL}
};

static cs_status uart_intr_status_clear(cs_uint8 uart_id)
{
...
static cs_int32 uart_intr_handler(cs_int32 vector, void *data)
{
    cs_uint8 uart_id;
    uart_intr_handler_t handler = NULL;

    switch (vector) {
        case ONU_INT_FLAGBIT_UART1:
            uart_id = 1;
            break;
        case ONU_INT_FLAGBIT_UART2:
            uart_id = 2;
            break;
        case ONU_INT_FLAGBIT_UART3:
            uart_id = 3;
            break;
        case ONU_INT_FLAGBIT_UART4:
            uart_id = 4;
            break;
        default:
            return CS_E_PARAM;
    ...
    uart_intr_status_clear(uart_id);

    handler = uart_channels[uart_id].intr_handler;
    if(handler)
        handler(uart_id,data);
...
cs_status uart_intr_handler_set(cs_uint8 uart_id, uart_intr_handler_t handler, cs_uint32 data)
{
    if((uart_id >= MAX_UART_NUM) || (NULL == handler))
        return CS_E_PARAM;

    uart_channels[uart_id].intr_handler = handler;

    cs_intr_reg(uart_channels[uart_id].isr_vector, "UART", uart_intr_handler, (void *)data, INTR_DSR_CB);

    return CS_E_OK;
}

cs_status uart_intr_handler_get(cs_uint8 uart_id, uart_intr_handler_t *handler)
{
    if(uart_id >= MAX_UART_NUM)
        return CS_E_PARAM;
...
cs_status uart_cfg_get(cs_uint8 uart_id, uart_cfg_t *config)
{
    PER_UART0_CFG_t  cfg;
    ...
    if((0 == cfg.bf.even_parity_sel) && (1 == cfg.bf.uart_parity_en))
        ...
        config->parity = UART_NONE_PARITY;
    ...
    if(fine_tun.bf.uart_half_duplex)
        config->duplex = UART_HALF_DUPLEX;
    else
        config->duplex = UART_FULL_DUPLEX;

    CS_REG_READ(uart_channels[uart_id].base_address + IMST_UARTIE, int_e.wrd);

    if(int_e.bf.tx_fifo_emptye | int_e.bf.rx_fifo_nempty)
        config->mode =  UART_INTR_MODE;
    else
        config->mode =  UART_POLLING_MODE;

    return CS_E_OK;

}

cs_status uart_cfg_set(cs_uint8 uart_id, uart_cfg_mask_t mask, uart_cfg_t *uart_cfg)
{
    PER_UART0_CFG_t cfg;
...
cs_status uart_ctrl_set(cs_uint8 uart_id, uart_ctrl_t ctrl)
{
    ...
    if((uart_id >= MAX_UART_NUM) || (ctrl > UART_ENABLE))
        return CS_E_PARAM;

    CS_REG_READ(uart_channels[uart_id].base_address + IMST_UARTIE, int_e.wrd);
    CS_REG_READ(uart_channels[uart_id].base_address + IMST_UARTCR, cfg.wrd);
    if(UART_DISABLE == ctrl)
    {
        int_e.bf.rx_fifo_nempty = 0;

        /* disable tx, rx and uart core*/
        cfg.bf.rx_sm_enable = 0;
        cfg.bf.tx_sm_enable = 0;
        cfg.bf.uart_en      = 0;
    }
    else
    {
    ...
    /* control config */
    CS_REG_WRITE(uart_channels[uart_id].base_address + IMST_UARTCR, cfg.wrd);
...
cs_boolean uart_getc_nonblock(cs_uint8 uart_id,cs_uint8* ch)
{
    cs_uint32 status ;
	#if 0
    int timeout = 0x30000;  /* ... just try twice*/
	#else
	int timeout = 0x2;  /* ... just try twice*/
	#endif
...
cs_status uart_putc(cs_uint8 uart_id,char ch)
{
        cs_uint32 status;
        int i;
...
{
	cs_thread_delay(5);
 	if(uart_getc_nonblock(uart_id, ch))
        return CS_E_OK;
    else
        return 1;
}
