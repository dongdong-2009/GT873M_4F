...
cs_status aal_ma_tx_ptp_no_mutex(cs_uint8 *buf, cs_uint16 len, cs_aal_port_id_t port, cs_uint8 pri)
{
    cs_uint32 data = 0;
    cs_uint32 temp = 0;
    __ma_dpid dpid;
    ...
    *buf_to++ = data;
    ...
    // trigger send
    REG_FIELD_WRITE(MA_CPU_CTRL, ptp_enable, 1);
    REG_FIELD_WRITE_NO_MUTEX(MA_TX_BUF_CTRL, pkt_rdy, 1);
    
     /* wait for buffer empty */
     time_out = __TX_BUF_TIMEOUT;
    do {
        REG_FIELD_READ(MA_TX_BUF_CTRL, pkt_rdy, busy);
    } while(busy && --time_out);
    if(busy)
    {
        AAL_CRI_LOG("wait for buffer empty timeout. FILE: %s, LINE: %d\n", __FILE__, __LINE__);
    ...
    REG_FIELD_WRITE(MA_CPU_CTRL, ptp_enable, 0);

    return CS_E_OK;
}


cs_status aal_ma_tx_ptp(cs_uint8 *buf, cs_uint16 len, cs_aal_port_id_t port, cs_uint8 pri)
{
    cs_status ret = CS_E_OK;
    cs_mutex_lock(__ma_tx_mutex);
    ret = aal_ma_tx_ptp_no_mutex(buf, len, port, pri);
    cs_mutex_unlock(__ma_tx_mutex); 
    return ret;
}
...
cs_status aal_ma_tx_no_mutex(cs_uint8 *buf, cs_uint16 len, cs_aal_port_id_t port, cs_uint8 pri)
{
    ...
    *buf_to++ = data;
    ...
    // trigger send
    REG_FIELD_WRITE_NO_MUTEX(MA_TX_BUF_CTRL, ptp_pkt, 0);
    REG_FIELD_WRITE_NO_MUTEX(MA_TX_BUF_CTRL, pkt_rdy, 1);
...
{    
    ...
    fw.u16 = 0;
#ifdef HAVE_FE
    fw.s.normal_pkt_fw  = AAL_MA_FW_FE; 
    fw.s.bpdu_pkt_fw    = AAL_MA_FW_FE;
    fw.s.dot1x_pkt_fw   = AAL_MA_FW_FE;
    fw.s.arp_pkt_fw     = AAL_MA_FW_FE;
    fw.s.oam_pkt_fw     = AAL_MA_FW_FE;
    fw.s.mymac_pkt_fw   = AAL_MA_FW_FE;
    fw.s.ipv4_pkt_fw    = AAL_MA_FW_FE;
    fw.s.ipv6_pkt_fw    = AAL_MA_FW_FE;
    fw.s.hello_pkt_fw   = AAL_MA_FW_FE;
#else
...
#ifdef HAVE_FE
    REG_FIELD_WRITE(MA_CPU_DST_EN_MII0, te_mii_pen0, 0);
    REG_FIELD_WRITE(MA_CPU_DST_EN_MII0, mii0, 0);
#endif
