...
/*
 * $Id: aal_pon.c,v 1.1.2.4.6.1.16.1.2.20.2.16.4.10 2011/09/14 09:20:47 duchen Exp $
 */
...
cs_status aal_pon_mib_get(cs_boolean read_clear, cs_aal_pon_mib_id_t mib_id, cs_uint32 *value_l, cs_uint32 *value_h)
{
    ...
    if(MIB_ID_PCS_ERROR_SYMBOL == mib_id)
    {
        ...
        retry = 0;
        while(0xdeadc0de == loc_pon_err_symbol && retry++ < 2)
        {    
            hal_delay_us(2);
            REG_FIELD_READ(ONU_MAC_PCS_Error_Symbol_Counter, Pcs_sym_err_cnt, loc_pon_err_symbol);
        }

        if(0xdeadc0de == loc_pon_err_symbol)
            loc_pon_err_symbol = 0;
    ...
    if(MIB_ID_PCS_OUTOFSYNC_CNT == mib_id)
    {
        ...
        retry = 0;
        while(0xdeadc0de == loc_pon_outofsync && retry++ < 2)
        {   
            hal_delay_us(2);
            REG_FIELD_READ(ONU_MAC_PCS_Error_Symbol_Counter, Pcs_sym_err_cnt, loc_pon_outofsync);
        }

        if(0xdeadc0de == loc_pon_outofsync)
            loc_pon_outofsync = 0;
...
)
{
    ...
    if(msk.s.preamble_mode)
    {
        REG_FIELD_WRITE(ONU_MAC_TXCTRL, reserved_1, (cfg->preamble_mode?1:0));
    }
...
cs_status pon_init(void)
{
    ...
    laser_cfg.tx_delay = 50; // for LynxD ASIC
