...
typedef union {
    struct {
        ...
        cs_uint32 set_timestamp_cmd : 1;
        cs_uint32 get_timestamp_cmd : 1;   /*0:HW latches PTP timer value;1:HW takes no action.*/
        cs_uint32 rsvd              : 28;
    } s;
    cs_uint32 u32;
...
cs_status aal_tod_info_set(
    CS_IN cs_uint8 len,
    CS_IN cs_uint8 *tod
);
cs_status aal_tod_info_get(
   CS_OUT cs_uint8 *len,
   CS_OUT cs_uint8 *tod
);

cs_status aal_tod_uart_set();

cs_status aal_tod_uart_get();
