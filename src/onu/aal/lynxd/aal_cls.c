...
/*********************************************/
...
#ifdef HAVE_FE
    def_fib.dpid.dst_op = AAL_CL_DST_L2;
#else
    ...
    ce_entry.u.cl_mirror_key[0].mirror_group_id = AAL_RATE_LIMIT_FLOW_20;
    ce_entry.u.cl_mirror_key[0].mirror_pkt_flag = 1;
    ce_entry.u.cl_mirror_key[0].mask           = CL_KEY_MASK_MIRROR_GROUP; 

    ret = cl_key_entry_set( AAL_FE_MA_ID, 15, &ce_entry);
    ...
    ret += cl_fib_table_set(AAL_FE_MA_ID, CL_SUB_KEY_NUM*15, &fibdata);
#endif
    return CS_E_OK;
}
