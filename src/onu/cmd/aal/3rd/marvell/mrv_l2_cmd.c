...
static void __sw_vlan_cmd_set(int argc, char **argv)
{
    ...
    InternalVlanInit(QD_MASTER_DEV_PTR, para1);

    for(i=0; i<QD_MASTER_DEV_PTR->maxPhyNum; i++)
    {
    	if(para2 & (1<<i))
    	{
    		if(para3&(1<<i))
    			InternalVlanJoin(QD_MASTER_DEV_PTR, para1, i, GT_FALSE);
    		else
    			InternalVlanJoin(QD_MASTER_DEV_PTR, para1, i, GT_TRUE);
    	}
//    	if(para3 & (1<<i))
//    		gprtSetEgressMode(QD_MASTER_DEV_PTR, i, GT_UNTAGGED_EGRESS);
    }
...
static void __sw_vlan_cmd_set(int argc, char **argv)
{
    ...
    for(i=0; i<QD_MASTER_DEV_PTR->maxPorts; i++)
    {
    ...
    gvtuAddEntry(QD_MASTER_DEV_PTR, &entry);
...
static void __sw_vlan_cmd_get(int argc, char **argv)
{
    ...
    if(gvtuFindVidEntry(QD_MASTER_DEV_PTR, &entry, &found) == GT_OK &&
    		found)
    {
    	cs_printf("fid is %d\n", entry.DBNum);
    	cs_printf("port list:\n");
    	for(i=0; i<QD_MASTER_DEV_PTR->maxPhyNum; i++)
    	{
    		if(entry.vtuData.memberTagP[i] == MEMBER_EGRESS_TAGGED)
    			cs_printf("%d(%s) ", i+1, "T");
    		if(entry.vtuData.memberTagP[i] == MEMBER_EGRESS_UNTAGGED)
    			cs_printf("%d(%s) ", i+1, "U");
    	}
    	cs_printf("\r\n");
    }
...
static void __sw_vlan_cmd_pvid_set(int argc, char **argv)
{
    ...
    rtn = gvlnSetPortVid(QD_MASTER_DEV_PTR, para1-1, para2);
    if (rtn)
        diag_printf("errcode(%d)\n", rtn);
    else
        diag_printf("done\n");

    rtn = gvlnSetPortVlanDBNum(QD_MASTER_DEV_PTR, para1-1, para2);
    if(rtn)
    	diag_printf("errcode(%d)\n", rtn);
    else
    	diag_printf("done\n");

    rtn = gstpSetPortState(QD_MASTER_DEV_PTR, para1-1, GT_PORT_FORWARDING);
    if(rtn)
    	diag_printf("errcode(%d)\n", rtn);
    else
    	diag_printf("done\n");
...
static void __sw_vlan_cmd_pvid_get(int argc, char **argv)
{
    ...
    rtn = gvlnGetPortVid(QD_MASTER_DEV_PTR, para1-1, &vlanid);
    if (rtn)
        diag_printf("errcode(%d)\n", rtn);
    else
        diag_printf("vlanid(%d)\n", vlanid);

    rtn = gstpGetPortState(QD_MASTER_DEV_PTR, para1-1, &state);
    if(rtn)
        diag_printf("errcode(%d)\n", rtn);
    else
        diag_printf("port state(%d)\n", state);

    return;
}

static void __sw_vlan_cmd_mode_set(int argc, char **argv)
{
    GT_STATUS rtn;
    ...
    if(argc == 2)
    rtn = gvlnSetPortVlanDot1qMode(QD_MASTER_DEV_PTR, port-1, mode);
    if(rtn)
        diag_printf("errcode(%d)\n", rtn);
...
static void __sw_vlan_cmd_mode_get(int argc, char **argv)
{
    GT_STATUS rtn;
    ...
    rtn = gvlnGetPortVlanDot1qMode(QD_MASTER_DEV_PTR, port-1, (GT_DOT1Q_MODE*)&mode);
    if (rtn)
        diag_printf("errcode(%d)\n", rtn);
    else
    {
        diag_printf("vlan mode %d\n", mode);
    }
...
static struct {
    ...
    char                   *help;
        ...
        { "mode-set",               __sw_vlan_cmd_mode_set       , 2,  "[port] [mode]", "set port's vlan mode"},
        { "mode-get",               __sw_vlan_cmd_mode_get       , 1,  "[port]", "get port's vlan mode"},
#if 0
...
static void __sw_l2_cmd_mac_learned_get(int argc, char **argv)
{
    ...
    rtn = gfdbGetPortAtuLearnCnt(QD_MASTER_DEV_PTR, para1-1, &learned_number);
    if (rtn)
        diag_printf("errcode(%d)\n", rtn);
    else
        diag_printf("mac-learned-number(%lu)\n", learned_number);
...
static void __sw_l2_cmd_mac_aging_enable_set(int argc, char **argv)
{
    ...
    rtn = gprtSetLearnDisable(QD_MASTER_DEV_PTR, para1-1, para2?GT_TRUE:GT_FALSE);
    if (rtn)
        diag_printf("errcode(%d)\n", rtn);
    else
        diag_printf("done\n");
...
static void __sw_l2_cmd_mac_aging_enable_get(int argc, char **argv)
{
    ...
    rtn = gprtGetLearnDisable(QD_MASTER_DEV_PTR, para1-1, &enable);
    if (rtn)
        diag_printf("errcode(%d)\n", rtn);
    else
        diag_printf("aging-enabled(%d:%s)\n", enable, enable ? "disabled":"enabled");
...
static void __sw_l2_cmd_mac_aging_time_set(int argc, char **argv)
{
    ...
    rtn = gfdbSetAgingTimeout(QD_MASTER_DEV_PTR, para1);
    if (rtn)
        diag_printf("errcode(%d)\n", rtn);
    else
        diag_printf("done\n");
...
static void __sw_l2_cmd_mac_aging_time_get(int argc, char **argv)
{
    ...
    rtn = gfdbGetAgingTimeout(QD_MASTER_DEV_PTR, &time);
    if (rtn)
        diag_printf("errcode(%d)\n", rtn);
    else
        diag_printf("aging-time(%lus)\n", time);
...
static void __sw_l2_cmd_mac_limit_set(int argc, char **argv)
{
    ...
    rtn = gfdbSetPortAtuLearnLimit(QD_MASTER_DEV_PTR, para1-1, para2);
    if (rtn)
        diag_printf("errcode(%d)\n", rtn);
    else
        diag_printf("done\n");
...
static void __sw_l2_cmd_mac_limit_get(int argc, char **argv)
{
    ...
    rtn = gfdbGetPortAtuLearnLimit(QD_MASTER_DEV_PTR, para1-1, &mac_limit);
    if (rtn)
        diag_printf("errcode(%d)\n", rtn);
    else
        diag_printf("mac-limit-number(%lu)\n", mac_limit);
...
static void __sw_l2_cmd_stp_state_set(int argc, char **argv)
{
    ...
    rtn = epon_request_onu_port_stp_state_set(context,0 , 0, para1, para2);
    if (rtn)
        diag_printf("errcode(%d)\n", rtn);
    else
        diag_printf("done\n");
...
static void __sw_l2_cmd_stp_state_get(int argc, char **argv)
{
    ...
    rtn = epon_request_onu_port_stp_state_get(context, 0,0, para1, &stp_state);
    if (rtn)
        diag_printf("errcode(%d)\n", rtn);
    else
...
{   
    ...
    rtn = epon_request_onu_mc_l2_entry_del(context,0,0,port,&entry);
    if (rtn)
        diag_printf("errcode(%d)\n", rtn);
    else
        diag_printf("done\n");
...
/*******************************************************************************/
extern void fdb_cmd_aging(int argc, char **argv);
//extern void fdb_cmd_static(int argc, char **argv);
extern void fdb_cmd_list_get(int argc,char **argv);
...
static struct {
    ...
    char                   *help;
        ...
        { "learn", 				fdb_cmd_learn 				, 2, "fdb learn <mode> <port>", "enable/disable mac learning 0: disable fdb learn  1: enable fdb learn"},
        { "aging", 				fdb_cmd_aging				, 1, "valid: <0s - 458s>   0: disable aging","aging timer. 0 - disable aging"},
        { "aging-get",				fdb_cmd_aging_get		, 0, "<cr>", "get fdb aging timer"},
