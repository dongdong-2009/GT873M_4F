...
void uni_cmd_mirror(int argc, char **argv)
{
#ifdef HAVE_RTK_CMD
    ...
    if(argc == 2)
    	{
    		 cs_printf("mirror <port> <rx_port_msk> <rx_port_msk>\n");
			 cs_printf("port num:\n");
			 cs_printf("   0x30: uplink port\n");
			 cs_printf("   1: UNI port 0\n");
			 cs_printf("   2: UNI port 1\n");
			 cs_printf("   3: UNI port 2\n");
			 cs_printf("   4: UNI port 3\n");
			 cs_printf("rx_port_mask:\n");
			 cs_printf("   bit 1: UNI port 0\n");
			 cs_printf("   bit 2: UNI port 1\n");
			 cs_printf("   bit 3: UNI port 2\n");
			 cs_printf("   bit 4: UNI port 3\n");
			 cs_printf("   bit 5: uplink port\n");
			 cs_printf("tx_port_mask:\n");
			 cs_printf("   bit 1: UNI port 0\n");
			 cs_printf("   bit 2: UNI port 1\n");
			 cs_printf("   bit 3: UNI port 2\n");
			 cs_printf("   bit 4: UNI port 3\n");
	...
	cs_printf("mir_port:%d rx_port:%d tx_port:%d\n",mirror_port,rx_port_msk,tx_port_msk);
...
sal_cmd_result_t port_cmd_drp(int argc, char **argv)
{
    ...
    dir = (cs_direction_t)iros_strtol(argv[2]);
    enable = iros_strtol(argv[3]);

    status = (cs_status)epon_request_onu_traffic_block(context, 0, 0, dir,enable);
    if(status != CS_E_OK){
        cs_printf("epon_request_onu_traffic_block failed! \n");    
        return cs_status_2_cmd_rc_map(status);       
    }
...
sal_cmd_result_t uni_cmd_storm_ctrl(int argc, char **argv)
{
	...
	cs_printf("port:%d---mode:%d---type:%d---rate.en:%d---rate:%d\n",portid,mode,type,rate.enable,rate.rate);
...
char *storm2string[] = 
{
    ...
    for(portid = CS_UNI_PORT_ID1; portid<= CS_DOWNLINK_PORT;portid++){
        if(portid > CS_UNI_PORT_ID4 && portid != CS_DOWNLINK_PORT){
            continue;
        }
#else
