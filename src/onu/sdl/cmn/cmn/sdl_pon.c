...
****************************************************************************/
/*
 * $Id: sdl_pon.c,v 1.1.2.5.2.20 2011/09/21 03:55:11 zsun Exp $
 */
    ...
    CS_IN cs_boolean              en)
{
    ...
    rt = cs_plat_gpio_write(context, 0, 0, __PON_TRAN_RX_POWER_CTRL_GPIO, status);
    if(rt)
    {
    ...
    CS_OUT cs_boolean             *en)
{
    ...
    rt = cs_plat_gpio_read(context, 0, 0, __PON_TRAN_RX_POWER_CTRL_GPIO, &status);
    if(rt)
    {
        SDL_MIN_LOG("onu_gpio_set_status return %d. FILE: %s, LINE: %d", rt, __FILE__, __LINE__);
        return CS_E_ERROR;
    ...
    CS_IN cs_boolean              en)
{
    ...
    rt = cs_plat_gpio_write(context, 0, 0, __PON_TRAN_TX_POWER_CTRL_GPIO, status);
    if(rt)
    {
        SDL_MIN_LOG("onu_gpio_set_status return %d. FILE: %s, LINE: %d", rt, __FILE__, __LINE__);
        return CS_E_ERROR;
    ...
    CS_OUT cs_boolean             *en)
{
    ...
    rt = cs_plat_gpio_read(context, 0, 0, __PON_TRAN_TX_POWER_CTRL_GPIO, &status);
    if(rt)
    {
        SDL_MIN_LOG("onu_gpio_set_status return %d. FILE: %s, LINE: %d", rt, __FILE__, __LINE__);
        return CS_E_ERROR;
...
)
{
    ...
    aal_pon_mib_get(0, MIB_ID_UCFRAMECNT            , &stats->pon_frame_cnt        , &value_h);
    aal_pon_mib_get(0, MIB_ID_PON_MCFRAMECNT        , &stats->pon_mcframe_cnt      , &value_h);
    aal_pon_mib_get(0, MIB_ID_PON_BCFRAMECNT        , &stats->pon_bcframe_cnt      , &value_h);   
}

static void __int_pon_link_change_handler(void)
{
