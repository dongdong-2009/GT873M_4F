...
****************************************************************************/
/*
 * $Id: sdl_ma.c,v 1.1.2.2.2.30.2.2 2012/01/11 09:24:27 panfeng Exp $
 */
...
cs_uint32 ma_ptp_mutex; /*for MA PTP enable use */
...
cs_status __ma_rx_process(cs_aal_ma_rx_buf_id_t buf_id, cs_aal_ma_rx_buf_status_t *buf_status)
{
    ...
    if(ma_debug)// DEBUG: dump packet
    {
        ...
        __cpu_packet_dump(buf.pkt->data+CS_PKT_OFFSET, buf_status->pkt_len);
    }
...
void rtk_fram_send(cs_uint8*LoopBackDetectPktBuffer,cs_uint8*pkt_buf,cs_uint16*buf_len,cs_uint32 port)
{
	__sw_pkt_tx_func(LoopBackDetectPktBuffer,64, pkt_buf,buf_len,port);
}
...
#if 0
cs_status epon_request_onu_frame_send_w (
    CS_IN cs_callback_context_t     context,
    CS_IN cs_int32                  device_id,
    CS_IN cs_int32                  llidport,
    CS_IN cs_uint8                  *frame_buf,
...
//   cs_printf("fram send:\n");
    ...
    aal_priority = priority;                                  
    ret = aal_ma_tx(frame_buf,frame_len,port,aal_priority);
    return ret;
}
#endif

cs_status epon_request_onu_ptp_sync_pkt_send (
    CS_IN cs_callback_context_t     context,
    CS_IN cs_int32                  device_id,
    CS_IN cs_int32                  llidport)
{
    ...
    cs_uint8 pkt[64] = 
    {
        0x01, 0x80, 0xc2, 0x00, 0x00, 0x20, 0x00, 0x00, 0x00, 0x00, 0x00, 0x08, 0x88, 0x99, 0x04, 0x00, 
        0x00, 0x00, 0x00, 0x40, 0x88, 0xf7, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    ret = aal_ma_tx(&pkt[0], 64, port, 7);
    
    return ret;
}

cs_status epon_request_onu_ptp_frame_send (
    CS_IN cs_callback_context_t     context,
    CS_IN cs_int32                  device_id,
    CS_IN cs_int32                  llidport,
    CS_IN cs_uint8                  *frame_buf,
    ...
    CS_IN cs_uint8                  priority)
{
    ...
    ret = aal_ma_tx_ptp(pkt_buf, pkt_len, port, aal_priority);
    return ret;
}
...
{
    cs_mutex_init(&ma_ptp_mutex, "ma_ptp_mutex", 0);
