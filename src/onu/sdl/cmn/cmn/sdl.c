...
void sdl_glb_int_process(void)
{
    cs_aal_ptp_int_msk_t src,en;

    aal_ptp_int_en_get(&en);
    aal_ptp_int_status_get(&src);

    if (en.s.get_ts_intI && src.s.get_ts_intI)
    {
        sdl_int_handler(SDL_INT_GLB_PTP_TS_GET);
    }

    if (en.s.set_ts_intI && src.s.set_ts_intI)
    {
        sdl_int_handler(SDL_INT_GLB_PTP_TS_SET);
    }

    aal_ptp_int_status_clr(src);
}
