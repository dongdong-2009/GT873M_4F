...
void ctc_onu_stats_monitor_init()
{
    ...
    for(i = 0; i <= CTC_MAX_ONU_UNI_PORTS; i++)
    {
		...
		g_moitor_stats[i].timer = 1;
		g_moitor_stats[i].interval= 10;
        ...
        oam_ctc_onu_port_stats_t * stats)
{
    ...
    else
    {
        ...
        stats->dn_drop_events            = 0;  
        stats->up_drop_events            = 0;      
        stats->dn_octets                 = uni_stas.rxbyte_cnt;      
        stats->up_octets                 = uni_stas.txbyte_cnt;      
        stats->dn_frames                 = uni_stas.rxucfrm_cnt + uni_stas.rxmcfrm_cnt + uni_stas.rxbcfrm_cnt;       
        stats->up_frames                 = uni_stas.txucfrm_cnt + uni_stas.txmcfrm_cnt + uni_stas.txbcfrm_cnt;       
        stats->dn_bc_frames              = uni_stas.rxbcfrm_cnt;         
        stats->up_bc_frames              = uni_stas.txbcfrm_cnt;         
        stats->dn_mc_frames              = uni_stas.rxmcfrm_cnt;          
        stats->up_mc_frames              = uni_stas.txmcfrm_cnt;          
        stats->dn_crc_err_frames         = uni_stas.rxcrcerrfrm_cnt;         
        stats->dn_undersize_frames       = uni_stas.rxundersizefrm_cnt;           
        stats->up_undersize_frames       = 0;           
        stats->dn_oversize_frames        = uni_stas.rxoversizefrm_cnt;          
        stats->up_oversize_frames        = uni_stas.txoversizefrm_cnt;          
        stats->dn_fragments              = 0;             
        stats->dn_jabbers                = uni_stas.rxjabberfrm_cnt;         
        ...
        stats->dn_64_octets_frames       = uni_stas.rxstatsfrm64_cnt;            
        stats->dn_65_127_octets_frames   = uni_stas.rxstatsfrm65_127_cnt;                
        stats->dn_128_255_octets_frames  = uni_stas.rxstatsfrm128_255_cnt;                
        stats->dn_256_511_octets_frames  = uni_stas.rxstatsfrm256_511_cnt;                 
        stats->dn_512_1023_octets_frames = uni_stas.rxstatsfrm512_1023_cnt;                 
        stats->dn_1024_1518_octets_frames= uni_stas.rxstatsfrm1024_1518_cnt;                  
        stats->up_64_octets_frames       = uni_stas.txstatsfrm64_cnt;                
        stats->up_65_127_octets_frames   = uni_stas.txstatsfrm65_127_cnt;                
        stats->up_128_255_octets_frames  = uni_stas.txstatsfrm128_255_cnt;                  
        stats->up_256_511_octets_frames  = uni_stas.txstatsfrm256_511_cnt;                 
        stats->up_512_1023_octets_frames = uni_stas.txstatsfrm512_1023_cnt;                  
        stats->up_1024_1518_octets_frames= uni_stas.txstatsfrm1024_1518_cnt;                   
        stats->dn_discard_frames         = 0;                 
        stats->up_discard_frames         = 0;              
        stats->dn_err_frames             = uni_stas.rxcrcerrfrm_cnt + uni_stas.rxundersizefrm_cnt + 
...
{
    cs_uint32 value = 0, raise_value = 0, clear_value  = 0;
        ...
        oam_ctc_onu_port_stats_t * stats)
{
	...
	stats->up_err_frames             = port_stats.up_crc_err_frames - ctrl->last_save.up_crc_err_frames;
...
enum{
	GWD_PORT_STATS_DEACTIVE,
	GWD_PORT_STATS_ACTIVE,
	GWD_PORT_STATS_UNKNOWN
};
static cs_uint8 port_stats_init_complete = 0;
cyg_uint64 time_stat = 0;
cs_uint32 next_port = 1,FIRST = 1;
...
void onu_tmfunc_port_stats_get(void*date)
{
	...
	if(next_get)
		{
			...
			for(i=0; i<uni_num; i++)
				{
					ptr_gwd_portstats_ctrl_t p = pctrl+i;
					next_port = 1;
					time_ticks = 0;
					time1_ticks =0;
					if(!p)
					...
					if(p->status == GWD_PORT_STATS_ACTIVE)
					{
						while(next_port)
							{
								if(FIRST)
								{
								 	epon_request_onu_port_stats_get(context, ONU_DEVICEID_FOR_API, ONU_LLIDPORT_FOR_API,p->portid,FALSE, p->phis);	
								...
								if(time_stat > 1000)
									{
										epon_request_onu_port_stats_get(context, ONU_DEVICEID_FOR_API, ONU_LLIDPORT_FOR_API,p->portid,FALSE, p->pcur);
										
										next_port = 0;
										FIRST = 1;
									}
							}
						rxrate = (cs_uint32)(pcur.rxbyte_cnt - phis.rxbyte_cnt)/time_stat;
						txrate = (cs_uint32)(pcur.txbyte_cnt - phis.txbyte_cnt)/time_stat;
						p->rxrate = rxrate;
						p->txrate = txrate;
    ...
    cs_timer_add(10, onu_tmfunc_port_stats_get, NULL);
}
#else
...
cs_boolean gwd_portstats_init()
{
	...
	if(uni_num > 0)
	{
		g_ps_ctrl = iros_malloc(IROS_MID_AAL, sizeof(gwd_portstats_ctrl_t)*uni_num);
		if(g_ps_ctrl)
		{
			cs_uint16 i = 0;
			for(i=0; i<uni_num; i++)
			{
				ptr_gwd_portstats_ctrl_t pctl = g_ps_ctrl+i;
				pctl->icount = 0;
				pctl->interval = PORT_STATS_DEFAULT_INTERVAL;
				pctl->phis = NULL;
				pctl->pcur = NULL;
				pctl->status = GWD_PORT_STATS_ACTIVE;
				pctl->portid = i+1;
				pctl->seconds = 0;
				pctl->portstatus = 0;
				pctl->linkchange = 0;
				pctl->linkchangehis = 0;
				cyg_mutex_init(&pctl->mutex);
			}
			ret = EPON_TRUE;
			port_stats_init_complete = 1;
		}
	}
	else
		printf("gwd_portstats_init fail! no uni port found!\n");

	return ret;
}

cs_boolean gwd_portstats_get_rate(cs_port_id_t portid, cs_uint64 *txrate, cs_uint64 * rxrate)
{
	cs_boolean ret = EPON_FALSE;

	if(!port_stats_init_complete)
		ret = gwd_portstats_init();
	else
		ret = EPON_TRUE;
	...
	if(ret == EPON_TRUE)
	{
		...
		ptr_gwd_portstats_ctrl_t pctrl = g_ps_ctrl+portid;

		if(pctrl)
		{
			cyg_mutex_lock(&pctrl->mutex);

			if(txrate && rxrate)
			{
				if(pctrl->status == GWD_PORT_STATS_ACTIVE)
				{
					*txrate = pctrl->txrate;
					*rxrate = pctrl->rxrate;
				}
