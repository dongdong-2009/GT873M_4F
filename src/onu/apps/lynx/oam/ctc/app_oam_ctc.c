        ...
        cs_uint8  *  pOut)
{
...
#ifdef HAVE_HW_OAM
    cs_uint8 vender[OAM_CTC_VENDER_ID_LEN] = {0x32, 0x30, 0x31, 0x31};
#ifdef HAVE_MPORTS
    cs_uint8 model[OAM_CTC_MODEL_ID_LEN] = {0x30, 0x00, 0x00, 0x00};
    cs_uint8 hwver[OAM_CTC_ONU_HW_LEN] = {0x48, 0x47, 0x38, 0x31, 0x33, 0x65, 0, 0};
#else
    cs_uint8 model[OAM_CTC_MODEL_ID_LEN] = {0x38, 0x31, 0x30, 0x65};
    cs_uint8 hwver[OAM_CTC_ONU_HW_LEN] = {0x48, 0x47, 0x38, 0x31, 0x30, 0x65, 0, 0};
#endif
    /*cs_uint8 swver[OAM_CTC_ONU_SW_LEN] = {0x56, 0x31, 0x30, 0x30, 0x52, 0x30, 0x30, 0x31, 0x43, 
        0x30, 0x31, 0x42, 0x30, 0x35, 0x32, 00};*/
    cs_uint8 swver[OAM_CTC_ONU_SW_LEN] = HW_OAM_APP_VERSION;/*V2R1C00{0x56,0x32,0x52,0x31,0x43,0x30,0x30,0x00};*/

    memcpy(pData->venderId,  vender, OAM_CTC_VENDER_ID_LEN);
    memcpy(pData->model,  model, OAM_CTC_MODEL_ID_LEN);
    ...
    ctc_oam_sn_get_adapt(&info);

    memcpy(pData->venderId + OAM_CTC_VENDER_ID_LEN - strlen(info.onu_vendor_id),  info.onu_vendor_id, strlen(info.onu_vendor_id));
    memcpy(pData->model + OAM_CTC_MODEL_ID_LEN - strlen(info.onu_model),  info.onu_model, strlen(info.onu_model));
    memcpy(pData->ONUId, info.onu_id, CS_MACADDR_LEN);

    memcpy(pData->hwVer + OAM_CTC_ONU_HW_LEN - strlen(info.hardware_ver),info.hardware_ver,strlen(info.hardware_ver));

    memset(swVer, 0x00,OAM_CTC_ONU_SW_LEN);
    memcpy(swVer,info.software_ver,OAM_CTC_ONU_SW_LEN);
    ...
    return sizeof(oam_ctc_onu_sn_t) ;
}
#else
cs_uint32 ctc_oam_sn_get(
        ctc_oam_mgmt_obj_idx_t index,
        ...
        cs_uint8  *  pOut)
{
    ...
    cs_uint8 vender[OAM_CTC_VENDER_ID_LEN] = {0x32, 0x30, 0x31, 0x31};
	#ifdef HAVE_MPORTS
	    cs_uint8 model[OAM_CTC_MODEL_ID_LEN] = {0x30, 0x81, 0x1d, 0x00};
	    cs_uint8 hwver[OAM_CTC_ONU_HW_LEN] = {0x48, 0x47, 0x38, 0x31, 0x33, 0x65, 0, 0};
	#else
	    cs_uint8 model[OAM_CTC_MODEL_ID_LEN] = {0x38, 0x31, 0x30, 0x65};
	    cs_uint8 hwver[OAM_CTC_ONU_HW_LEN] = {0x48, 0x47, 0x38, 0x31, 0x30, 0x65, 0, 0};
...
#else
	//cs_uint8 model[OAM_CTC_MODEL_ID_LEN] = {0x30, 0x81, 0x10, 0x64};
  //  cs_uint8 swver[OAM_CTC_ONU_SW_LEN] = HW_OAM_APP_VERSION;
    memcpy(pData->venderId, info.onu_vendor_id, OAM_CTC_VENDER_ID_LEN);
    memcpy(pData->model, info.onu_model, OAM_CTC_MODEL_ID_LEN);
    memcpy(pData->hwVer, info.hardware_ver, OAM_CTC_ONU_HW_LEN);
    memcpy(pData->swVer, info.software_ver, OAM_CTC_ONU_SW_LEN);
    memcpy(pData->ONUId, mac, CS_MACADDR_LEN);
#endif
        ...
        cs_uint8  *  pOut)
{
    ...
    if(oam_is_cuc_oam()){
        ...
        ctc_oam_fw_get_adapt(&img_ver_info);
        pData->hdr.width = sizeof(oam_image_version_t);
        img_ver_info.major = htons(img_ver_info.major);
        img_ver_info.minor = htons(img_ver_info.minor);
        img_ver_info.revision = htons(img_ver_info.revision);
        img_ver_info.build_id = htons(img_ver_info.build_id);

        memcpy(pData->fwVersion, &img_ver_info, sizeof(oam_image_version_t));
    }
#endif
        ...
        cs_uint8  *  pOut)
{
...
#ifdef HAVE_HW_OAM
    cs_uint8 temp[8] = {0x45,0x36,0x63,0x01,0,0x6,0x9,0x1b};
    pData->hdr.width = 8;
    ...
    ctc_oam_chipid_get_adapt(&info);

    if(oam_is_cuc_oam()){
        sscanf(info.date,"%2d",&a);
        sscanf(info.date,"%*[^/]/%2d",&b);
        sscanf(info.date,"%*[^/]/%*[^/]/%2d",&c);
    }else{
        sscanf(info.date,"%2x",&a);
        sscanf(info.date,"%*[^/]/%2x",&b);
        sscanf(info.date,"%*[^/]/%*[^/]/%2x",&c);
    }   
    ...
    memcpy(pData->venderId,  info.chip_vendor_id, OAM_CTC_ONU_CHIPID_VID_LEN);
    memcpy(pData->chipModel, info.chip_model, OAM_CTC_ONU_CHIPID_MODEL_LEN);
    pData->Revision = info.rev;
#endif    
...
#ifdef HAVE_MPORTS
    pData->intf[0].interfaceType = htonl(1); /* GE */
#else
    pData->intf[0].interfaceType = htonl(0); /* FE */
#endif
        ...
        cs_uint8  *  pOut)
{
    ...
    OAM_CTC_FUNCTION_ENTER(index);
    diag_printf("ctc_oam_holdover_get\n");
        ...
        cs_uint8  *  pOut)
{
    ...
    OAM_CTC_FUNCTION_ENTER(index);
	memset(&mgmt,0,sizeof(ctc_oam_mgmt_parm_t));
        ...
        cs_uint8 * pOut)
{
	...
	//epon_request_onu_port_link_status_get(context, 0, 0, index.idxValue, &state);
...
#ifdef HAVE_HW_OAM
    if(!intf || intf->ctc_OAM_Ext_version < 0x21)
    {
        OAM_ORG_LOG("Not support for this version \n");
        pData->hdr.width  = OAM_CTC_VAR_SET_BAD_PARAM; 
        return sizeof(oam_var_cont_t);
    }
#endif
        ...
        cs_uint8   * pOut)
{
    ...
    OAM_CTC_FUNCTION_ENTER(index);
	diag_printf("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^6ctc_oam_onu_port_mode_set\n");
    if(!oam_is_cuc_oam()){
        OAM_ORG_LOG("CTC OAM, not support it \n");
        pOutData->width  = OAM_CTC_VAR_SET_BAD_PARAM; 
        return sizeof(oam_var_cont_t);
...
cs_uint32 ctc_oam_onu_mctagstrip_set(
        ...
        cs_uint8   * pOut)
{
	//cs_printf("ctc_oam_onu_mctagstrip\n");
    cs_status ret = CS_E_OK;
    oam_var_cont_t *pOutData
        = (oam_var_cont_t *)pOut;
...
{
#if 0
	cs_printf("in ctc_oam_onu_mc_switch_set\n");
#endif
    cs_status ret = CS_E_OK;
    oam_var_cont_t *pOutData
        = (oam_var_cont_t *)pOut;
        ...
        cs_uint8 * pOut)
{
    ...
    oam_var_cont_val_t *pNextTlv = NULL;
    cs_status ret = CS_E_OK;
    ...
    pData->stats.dn_drop_events            = htonl( stats.dn_drop_events            );  
    pData->stats.up_drop_events            = htonl( stats.up_drop_events            );      
    pData->stats.dn_octets                 = htonl( stats.dn_octets                 );      
    pData->stats.up_octets                 = htonl( stats.up_octets                 );      
    pData->stats.dn_frames                 = htonl( stats.dn_frames                 );       
    pData->stats.up_frames                 = htonl( stats.up_frames                 );       
    pData->stats.dn_bc_frames              = htonl( stats.dn_bc_frames              );         
    pData->stats.up_bc_frames              = htonl( stats.up_bc_frames              );         
    pData->stats.dn_mc_frames              = htonl( stats.dn_mc_frames              );          
    pData->stats.up_mc_frames              = htonl( stats.up_mc_frames              );          
    pData->stats.dn_crc_err_frames         = htonl( stats.dn_crc_err_frames         );         
    pData->stats.dn_undersize_frames       = htonl( stats.dn_undersize_frames       );           
    pData->stats.up_undersize_frames       = htonl( stats.up_undersize_frames       );           
    pData->stats.dn_oversize_frames        = htonl( stats.dn_oversize_frames        );          
    pData->stats.up_oversize_frames        = htonl( stats.up_oversize_frames        );          
    pData->stats.dn_fragments              = htonl( stats.dn_fragments              );             
    pData->stats.dn_jabbers                = htonl( stats.dn_jabbers                );         
    pData->stats.dn_collisions             = htonl( stats.dn_collisions             );             
    pData->stats.dn_64_octets_frames       = htonl( stats.dn_64_octets_frames       );            
    pData->stats.dn_65_127_octets_frames   = htonl( stats.dn_65_127_octets_frames   );                
    pData->stats.dn_128_255_octets_frames  = htonl( stats.dn_128_255_octets_frames  );                
    pData->stats.dn_256_511_octets_frames  = htonl( stats.dn_256_511_octets_frames  );                 
    pData->stats.dn_512_1023_octets_frames = htonl( stats.dn_512_1023_octets_frames );                 
    pData->stats.dn_1024_1518_octets_frames= htonl( stats.dn_1024_1518_octets_frames);                  
    pData->stats.up_64_octets_frames       = htonl( stats.up_64_octets_frames       );                
    pData->stats.up_65_127_octets_frames   = htonl( stats.up_65_127_octets_frames   );                
    pData->stats.up_128_255_octets_frames  = htonl( stats.up_128_255_octets_frames  );                  
    pData->stats.up_256_511_octets_frames  = htonl( stats.up_256_511_octets_frames  );                 
    pData->stats.up_512_1023_octets_frames = htonl( stats.up_512_1023_octets_frames );                  
    pData->stats.up_1024_1518_octets_frames= htonl( stats.up_1024_1518_octets_frames);                   
    pData->stats.dn_discard_frames         = htonl( stats.dn_discard_frames         );                 
    pData->stats.up_discard_frames         = htonl( stats.up_discard_frames         );              

    totalLen = sizeof(oam_var_cont_t);

    pData->hdr.width = sizeof(oam_ctc_onu_port_stats_t) - 2 * sizeof(cs_uint32);
    totalLen += pData->hdr.width ;

    pNextTlv = (oam_var_cont_val_t*) (pOut + totalLen) ;
    pNextTlv->hdr.branch = pInData->branch; 
    pNextTlv->hdr.leaf = pInData->leaf; 

    totalLen += sizeof(oam_var_cont_t);
    pNextTlv->value1 = htonl( stats.dn_err_frames             ); 
    pNextTlv->value2 = htonl( stats.status_change_times       ); 
    pNextTlv->hdr.width = sizeof(cs_uint32)*2;

    totalLen += pNextTlv->hdr.width;
        ...
        cs_uint8 * pOut)
{
    ...
    pData->hdr.width = 0;
    totalLen += sizeof(oam_var_cont_t)+sizeof(pData->frag0) ;
    for(i=0; i<sizeof(pData->frag0)/sizeof(cs_uint64); i++)
    {
    	cs_uint64 *p = (cs_uint64*)&stats;
    	pData->frag0[i] = htonll(p[i]);
    }

    pData->hdr0.branch = pInData->branch;
    pData->hdr0.leaf = pInData->leaf;
    pData->hdr0.width = 0;
    for(i=0; i<sizeof(pData->frag1)/sizeof(cs_uint64); i++)
    {
    	cs_uint64 *p = ((cs_uint64*)&stats) + (sizeof(pData->frag0)/sizeof(cs_uint64));
    	pData->frag1[i] = htonll(p[i]);
    }
    totalLen += sizeof(oam_var_cont_t)+sizeof(pData->frag1);

    pData->hdr1.branch = pInData->branch;
    pData->hdr1.leaf = pInData->leaf;
    pData->hdr1.width = sizeof(pData->frag2);
    pData->frag2[0] = htonll(stats.dn_discard_frames);
    pData->frag2[1] = htonll(stats.up_discard_frames);
    pData->frag2[2] = htonll(stats.dn_err_frames);
    pData->frag2[3] = htonll(stats.up_err_frames);
    pData->frag2[4] = htonll(stats.status_change_times);

    totalLen += pData->hdr1.width + sizeof(oam_var_cont_t);
        ...
        cs_uint8   * pOut)
{
    ...
    stats.dn_drop_events            = ntohl( pInData->stats.dn_drop_events            );
    stats.up_drop_events            = ntohl( pInData->stats.up_drop_events            );
    stats.dn_octets                 = ntohl( pInData->stats.dn_octets                 );
    stats.up_octets                 = ntohl( pInData->stats.up_octets                 );
    stats.dn_frames                 = ntohl( pInData->stats.dn_frames                 );
    stats.up_frames                 = ntohl( pInData->stats.up_frames                 );
    stats.dn_bc_frames              = ntohl( pInData->stats.dn_bc_frames              );
    stats.up_bc_frames              = ntohl( pInData->stats.up_bc_frames              );
    stats.dn_mc_frames              = ntohl( pInData->stats.dn_mc_frames              );
    stats.up_mc_frames              = ntohl( pInData->stats.up_mc_frames              );
    stats.dn_crc_err_frames         = ntohl( pInData->stats.dn_crc_err_frames         );
    stats.dn_undersize_frames       = ntohl( pInData->stats.dn_undersize_frames       );
    stats.up_undersize_frames       = ntohl( pInData->stats.up_undersize_frames       );
    stats.dn_oversize_frames        = ntohl( pInData->stats.dn_oversize_frames        );
    stats.up_oversize_frames        = ntohl( pInData->stats.up_oversize_frames        );
    stats.dn_fragments              = ntohl( pInData->stats.dn_fragments              );
    stats.dn_jabbers                = ntohl( pInData->stats.dn_jabbers                );
    stats.dn_collisions             = ntohl( pInData->stats.dn_collisions             );
    stats.dn_64_octets_frames       = ntohl( pInData->stats.dn_64_octets_frames       );
    stats.dn_65_127_octets_frames   = ntohl( pInData->stats.dn_65_127_octets_frames   );
    stats.dn_128_255_octets_frames  = ntohl( pInData->stats.dn_128_255_octets_frames  );
    stats.dn_256_511_octets_frames  = ntohl( pInData->stats.dn_256_511_octets_frames  );
    stats.dn_512_1023_octets_frames = ntohl( pInData->stats.dn_512_1023_octets_frames );
    stats.dn_1024_1518_octets_frames= ntohl( pInData->stats.dn_1024_1518_octets_frames);
    stats.up_64_octets_frames       = ntohl( pInData->stats.up_64_octets_frames       );
    stats.up_65_127_octets_frames   = ntohl( pInData->stats.up_65_127_octets_frames   );
    stats.up_128_255_octets_frames  = ntohl( pInData->stats.up_128_255_octets_frames  );
    stats.up_256_511_octets_frames  = ntohl( pInData->stats.up_256_511_octets_frames  );
    stats.up_512_1023_octets_frames = ntohl( pInData->stats.up_512_1023_octets_frames );
    stats.up_1024_1518_octets_frames= ntohl( pInData->stats.up_1024_1518_octets_frames);
    stats.dn_discard_frames         = ntohl( pInData->stats.dn_discard_frames         );
    stats.up_discard_frames         = ntohl( pInData->stats.up_discard_frames         );
    stats.dn_err_frames             = ntohl( pInData->stats.dn_err_frames             );
    stats.status_change_times       = ntohl( pInData->stats.status_change_times       );
    ctc_oam_onu_stats_dump(&stats);
        ...
        if(lastBranch == pRecvVarCont->branch 
            ...
            if(lastBranch != OAM_ATTRIB_BRANCH_NULL 
               ...
               /* process the pLastRecvVarCont */
                memset(outPdu, 0, OAM_MAX_OAMPDU_SIZE);
                len = ctc_oam_build_set_tlv(pRecvPkt->oui, outPdu,pLastRecvVarCont,
    ...
    switch (pInAuth->auth_code)
    {
        case OAM_CTC_AUTH_REQUEST:
        {
            ...
            if(OAM_CTC_AUTH_TYPE_LOID_PASSWD ==pInAuthData->auth_type){
                ...
                if (  DB_CTC_LOID_MAX_LEN-loid_len!=0) {
                    memset(pOutAuthData->auth_data, 0, DB_CTC_LOID_MAX_LEN-loid_len);
                }
                memcpy(pOutAuthData->auth_data+DB_CTC_LOID_MAX_LEN-loid_len, db_loid.loid, loid_len);

                if (DB_CTC_PWD_MAX_LEN -passwd_len !=0) {
                    memset(pOutAuthData->auth_data+DB_CTC_LOID_MAX_LEN, 0, DB_CTC_PWD_MAX_LEN -passwd_len );
                }
                memcpy(pOutAuthData->auth_data+DB_CTC_LOID_MAX_LEN+DB_CTC_PWD_MAX_LEN -passwd_len, db_loid.passwd, passwd_len);
#endif
    ...
    switch(pUpgradeComm->opcode)
    {
        ...
        case CTC_OAM_ONU_UPGRADE_FILE_TRANSFER_DATA:
        {
            ...
            if(p_data->block == 1)
            {
                cs_uint8 *tmp = NULL;
                /*check signature*/
                if(strncmp(ONU_IMG_SIGNATURE_STR2, p_data->data, 7))
                {
                     p_outpacket->length += ctc_oam_upgrade_error_build(pErrMsg, TFTP_ERROR_ILLEGAL_OPERTION, 
                                                            g_ctc_image_upgrade_err_msg[2]);
                     img_upgrade_download_stop(0);
                     
                     break;
                }
                tmp = p_data->data + 7 + 4;
                memmove(p_data->data, tmp, tftp_len-4-11);
                tftp_len = tftp_len - 11;
            }
            #endif
        ...
        case CTC_OAM_ONU_UPGRADE_ACTIVATE_IMG_REQ:
        {
            ...
            if(g_ctc_upgrade_status.status == CTC_IMAGE_UPGRADE_ACTIVE)
            {
