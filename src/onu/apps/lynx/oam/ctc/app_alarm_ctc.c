...
ctc_oam_alarm_info_t g_pon_alarm_cfg[] = 
{
    ...
    {CTC_ALARM_PON_RX_COLLIS,  CTC_THRESH_UNSIGNED_HIGH,   TRUE,           FALSE},
    ...
    {CTC_WARN_PON_RX_COLLIS,  CTC_THRESH_UNSIGNED_HIGH,   TRUE,           FALSE},
...
/*****************************************************************************/
{
    ...
    ctc_oam_alarm_info_t item_cfg[] = 
    {
        ...
        {CTC_ALARM_PORT_RX_COLLIS,  CTC_THRESH_UNSIGNED_HIGH,   TRUE,        FALSE  },
        ...
        {CTC_WARN_PORT_RX_COLLIS,  CTC_THRESH_UNSIGNED_HIGH,    TRUE,        FALSE  },
        ...
        void        *data)
{
        ...
        cs_log_msg(IROS_LOG_LEVEL_MIN, IROS_MID_ORG_OAM, "port %d, link %d\n", port, opr);
        ...
        void        *data)
{
        ...
        cs_log_msg(IROS_LOG_LEVEL_MIN, IROS_MID_ORG_OAM, "port %d, status %d\n", port, status);
...
/*****************************************************************************/
{
        ...
        len +=  sizeof(oam_ctc_tlv_org_spec_t) - sizeof(alarm_info); /*remove seq_num field*/
        p_tlv_org_event_oam ->type = OAM_INFO_TLV_ORG_SPEC;
        p_tlv_org_event_oam ->length = sizeof(oam_ctc_tlv_org_spec_t) - sizeof(alarm_info);/*remove seq_num field*/
        ...
        if(p_info->info_need)
        {
            alarm_info = htonl(alarm_info);
            memcpy(p_tlv_org_event_oam->org_spec_value.alarm_info,&alarm_info,sizeof(cs_uint32));
            len += sizeof(cs_uint32);
            p_tlv_org_event_oam ->length += sizeof(cs_uint32);
        }
...
/*****************************************************************************/
{
    ...
    ret = cs_plat_i2c_opm_status_read(context,0,0,&tem, &svcc, &tx_bias, &tx_power, &rx_power);
            ...
            if(p_info[i].threshold_type != CTC_THRESH_UNSUPPORT)
            {
                ...
                switch(p_info[i].threshold_type)
                {
                    case CTC_THRESH_SIGNED_HIGH:
                        ...
                        {
                            ctc_oam_build_alarm_event(type, port, p_info[i].alarm_code, CTC_ALARM_RAISE, sign_value);
                        }
                        else if(sign_value <  p_info[i].clear_alarm_threshold.signValue)
                        {
                            ctc_oam_build_alarm_event(type, port, p_info[i].alarm_code, CTC_ALARM_CLEAR, sign_value);
                        }
                        break;
                    case CTC_THRESH_UNSIGNED_HIGH:
                        if(unsign_value >= p_info[i].raise_alarm_threshold.unsignValue)
                        {
                            ctc_oam_build_alarm_event(type, port, p_info[i].alarm_code, CTC_ALARM_RAISE, unsign_value);
                        }
                        else if(unsign_value <  p_info[i].clear_alarm_threshold.unsignValue)
                        {
                            ctc_oam_build_alarm_event(type, port, p_info[i].alarm_code, CTC_ALARM_CLEAR, unsign_value);
                        }
                        break;
