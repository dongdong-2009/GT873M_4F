        ...
        cs_uint8 action)
{
    ...
    cs_direction_t     dir = CS_UP_STREAM;
    cs_sdl_pkt_dst_t   state = DST_FE;
    ...
    {
        ret = epon_request_onu_spec_pkt_dst_set(context, 0, 0, CS_UP_STREAM, CS_PKT_PPPOE, state);
        ret = epon_request_onu_spec_pkt_dst_set(context, 0, 0, CS_DOWN_STREAM, CS_PKT_PPPOE, state);
    }
    else
    {
        ret = epon_request_onu_spec_pkt_dst_set(context, 0, 0, dir, CS_PKT_PPPOE, state);
    }
        ...
        cs_boolean enable)
{
    ...
    PPPOE_LOG_DEBUG("set port lpbk:port=%d, mode=%d\n", port, mode);
    if(CS_E_OK != ret)
    {
        PPPOE_LOG("set port lpbk fail, ret=%d\n", ret);
    }
        ...
        cs_sdl_stp_state_t *state)
{
    ...
    ret = epon_request_onu_port_stp_state_get(context, 0, 0, port, state);
    if(ret != CS_E_OK){
