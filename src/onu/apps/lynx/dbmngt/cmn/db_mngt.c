...
cs_status db_table_create()
{
    ...
    if(CS_E_OK != db_load_from_flash()){
        cs_printf("Load configuration from flash failed or flash configuration has not initialized.\nUse the factory configuration. \n");
        memset(g_db_info.data_pool, 0, g_db_info.pool_size);
        reset_default = TRUE;
    }
    ...
    if(!reset_default 
            && g_db_info.cur_version != g_db_info.flash_version 
            && g_DbUpgrade != NULL){
        g_DbUpgrade(g_db_info.flash_version);
    }

    end = cs_current_time();
    cs_printf("db create cost %d tick\n", (end-begin));
...
cs_status db_load_from_flash()
{
    ...
    if(htonl(TLV_HEADER_MAGIC) != header->magic){
        status = CS_ERROR;
        goto ERROR;
    }
