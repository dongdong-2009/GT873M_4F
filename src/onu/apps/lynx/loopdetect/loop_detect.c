...
cs_status loop_detect_unblock(cs_port_id_t port)
{
    ...
    if(g_ldCb.block_obj == LOOP_DETECT_BLOCK_GLOBAL)
    {
        if(g_ldCb.block_state == LOOP_DETECT_BLOCKED)
        {
            ...
            for(dest_port=1; dest_port<=LOOP_DETECT_PORT_MAX; dest_port++)
            {
                result = epon_request_onu_port_stp_state_set(context, ONU_DEVICEID_FOR_API, 
                                                                                  ONU_LLIDPORT_FOR_API, dest_port, STP_S_FORWARDING);
            }
        ...
        for(i = 0; i < LOOP_DETECT_PORT_MAX; i++)
        {
            if((i == port-1) || (CS_ALL_UNI_PORT_ID == port))
            {
                if(g_ldCb.detect_ports[i].block_state == LOOP_DETECT_BLOCKED)
                {
                    ...
                    /*open port*/
                    result = epon_request_onu_port_stp_state_set(context, ONU_DEVICEID_FOR_API, 
                                                                                       ONU_LLIDPORT_FOR_API, i+1, STP_S_FORWARDING);
                }
            }
        }
    }
...
void loop_detect_init()
{
    ...
    for(i = 0; i < LOOP_DETECT_PORT_MAX; i++)
    {
        ...
        g_ldCb.detect_ports[i].block_state = LOOP_DETECT_NOT_BLOCK;
        g_ldCb.detect_ports[i].need_block = LOOP_DETECT_BLOCK_ENABLE;
...
cs_status loop_detect_period_common_handle(cs_port_id_t port)
{
    ...
    for(i = 0; i < LOOP_DETECT_PORT_MAX; i++)
    {
        ...
        {
            epon_request_onu_port_admin_get(context,0,0,
            ...
            if (port_admin_status == SDL_PORT_ADMIN_UP)
            {
                ...
                /*open for this port*/
                epon_request_onu_port_stp_state_set(context, ONU_DEVICEID_FOR_API, ONU_LLIDPORT_FOR_API, i+1, STP_S_FORWARDING);
                g_ldCb.detect_ports[i].block_state = LOOP_DETECT_NOT_BLOCK;
                IROS_LOG_MSG(IROS_LOG_LEVEL_MIN, IROS_MID_APP, 
                                        "open port %d period time out\n", i+1);
                
                frame.lb_port = htonl(i+1);
                DUMPLOOPPKT("ld send", (cs_uint8*)&frame, sizeof(frame));

                /*send frame with valn*/
                vlan_flooding_per_port(i+1, ONU_CPU_PKT_PRIO, (cs_uint8*)&frame, sizeof(frame));

                if(g_ldCb.detect_ports[i].status == LOOP_DETECTED)
                {
                    IROS_LOG_MSG(IROS_LOG_LEVEL_MIN, IROS_MID_APP,  "need loop check \n");
                ...
                else if(g_ldCb.detect_ports[i].status == LOOP_CHECK)
                {
                    ...
                    result = epon_request_onu_port_stp_state_set(context, ONU_DEVICEID_FOR_API, ONU_LLIDPORT_FOR_API, i+1, STP_S_FORWARDING);
                    /*after handler*/
                    LOOP_DETECT_CALL_BACK(i+1, LOOP_CLEARED_AFTER_EVENT);
                    IROS_LOG_MSG(IROS_LOG_LEVEL_MIN, IROS_MID_APP,  "Loop cleared \n");
...
cs_status loop_detect_enable_common_handle(cs_port_id_t port)
{
    ...
    if(!g_ldCb.enable)
    {
        ...
        epon_request_onu_spec_pkt_dst_set(context, ONU_DEVICEID_FOR_API, ONU_LLIDPORT_FOR_API, 
                                                                    CS_UP_STREAM, CS_PKT_LOOPDETECT, DST_CPU);
#if 0
...
cs_status loop_clear_check_common_handle(cs_port_id_t port)
{
    ...
    if(g_ldCb.detect_ports[port-1].status == LOOP_CHECK)
    {
        ...
        /*open this port anyway*/
        epon_request_onu_port_stp_state_set(context, ONU_DEVICEID_FOR_API, ONU_LLIDPORT_FOR_API, port, STP_S_FORWARDING);
        g_ldCb.detect_ports[port-1].status = NO_LOOP;

        /*loop cleared after handle*/
        LOOP_DETECT_CALL_BACK(port, LOOP_CLEARED_AFTER_EVENT);
        IROS_LOG_MSG(IROS_LOG_LEVEL_MIN, IROS_MID_APP,  "Loop cleared when  immediate check\n");
    }
...
cs_status loop_detect_disable_common_handle(cs_port_id_t port)
{
    ...
    if(!exist)
    {
        ...
        epon_request_onu_spec_pkt_dst_set(context, ONU_DEVICEID_FOR_API, ONU_LLIDPORT_FOR_API, 
                                                                    CS_UP_STREAM, CS_PKT_LOOPDETECT, DST_FE);
        #if 0
...
cs_status loop_detected_common_handle(cs_port_id_t port)
{
    ...
    if(need_block)
    {
        ...
        if(g_ldCb.block_obj == LOOP_DETECT_BLOCK_GLOBAL)
        {
            if(g_ldCb.block_state != LOOP_DETECT_BLOCKED)
            {
                ...
                {
                    result = epon_request_onu_port_stp_state_set(context, ONU_DEVICEID_FOR_API, 
                                                                                      ONU_LLIDPORT_FOR_API, dest_port, STP_S_BLOCKING);
                }
                IROS_LOG_MSG(IROS_LOG_LEVEL_MIN, IROS_MID_APP, "block uni for receive frame\n");
        ...
        else
        {
            ...
            {
                result = epon_request_onu_port_stp_state_set(context, ONU_DEVICEID_FOR_API, 
                                                                                  ONU_LLIDPORT_FOR_API, port, STP_S_BLOCKING);
                IROS_LOG_MSG(IROS_LOG_LEVEL_MIN, IROS_MID_APP, 
    ...
    if(g_ldCb.detect_ports[port-1].status == NO_LOOP)
    {
        ...
        IROS_LOG_MSG(IROS_LOG_LEVEL_MIN, IROS_MID_APP,  "port(%d) loop detected \n", port);
    }
    else
    {
        g_ldCb.detect_ports[port-1].status = LOOP_DETECTED;
...
cs_status loop_detect_block_expired_common_handle(cs_port_id_t port)
{
    ...
    for(dest_port=1; dest_port<=LOOP_DETECT_PORT_MAX; dest_port++)
    {
        if(dest_port == port || port == CS_ALL_UNI_PORT_ID)
        {
            result = epon_request_onu_port_stp_state_set(context, ONU_DEVICEID_FOR_API, ONU_LLIDPORT_FOR_API, dest_port, STP_S_FORWARDING);
        }
    }
    IROS_LOG_MSG(IROS_LOG_LEVEL_MIN, IROS_MID_APP, 
...
void loop_detect_port_oper_down_handler(cs_port_id_t port)
{
    ...
    IROS_LOG_MSG(IROS_LOG_LEVEL_MIN, IROS_MID_APP, "port(%d) oper down handle\n", port);
    ...
    {
        IROS_LOG_MSG(IROS_LOG_LEVEL_MIN, IROS_MID_APP, "ignore it\n");
        return;
    }
    ...
    if(g_ldCb.block_obj == LOOP_DETECT_BLOCK_PORT_BASE)
    {
        if(g_ldCb.detect_ports[port-1].status != NO_LOOP)
        {
            ...
            g_ldCb.detect_ports[port-1].status = NO_LOOP;
            /*open this port anyway*/
            epon_request_onu_port_stp_state_set(context, ONU_DEVICEID_FOR_API, 
                                                                               ONU_LLIDPORT_FOR_API, port, STP_S_FORWARDING);
            /*after handler*/
            LOOP_DETECT_CALL_BACK(port, LOOP_CLEARED_AFTER_EVENT);
...
cs_status loop_detect_event_handler(cs_port_id_t port, loop_detect_event_t  type)
{
    ...
    switch(type)
    {
        case LOOP_DETECTED_EVENT:
            ...
            IROS_LOG_MSG(IROS_LOG_LEVEL_MIN, IROS_MID_APP, "LOOP_DETECTED:port(%d)\n", port);
            result = loop_detected_common_handle(port);
            
            break;

        case LOOP_CLEARED_EVENT:
            IROS_LOG_MSG(IROS_LOG_LEVEL_MIN, IROS_MID_APP, "LOOP_CLEARED:port(%d)\n", port);
            result = loop_cleared_common_handle(port);
            break;
            
        case UNBLOCK_TIMER_EXPIRED_EVENT:
            IROS_LOG_MSG(IROS_LOG_LEVEL_MIN, IROS_MID_APP, "UNBLOCK_TIMER_EXPIRED:port(%d)\n", port);
            result = loop_detect_block_expired_common_handle(port);
            break;
            
            
        case LOOPDETECT_TIMER_EXPIRED_EVENT:
            IROS_LOG_MSG(IROS_LOG_LEVEL_MIN, IROS_MID_APP, "LOOPDETECT_TIMER_EXPIRED:port(%d)\n", port);
            result = loop_detect_period_common_handle(port);
            break;
            
        case LOOPDETECT_ENABLE_EVENT:
            IROS_LOG_MSG(IROS_LOG_LEVEL_MIN, IROS_MID_APP, "LOOPDETECT_ENABLE:port(%d)\n", port);
            result = loop_detect_enable_common_handle(port);
            break;
            
        case LOOPDETECT_DISABLE_EVENT:
            IROS_LOG_MSG(IROS_LOG_LEVEL_MIN, IROS_MID_APP, "LOOPDETECT_DISABLE:port(%d)\n", port);
            loop_detect_disable_common_handle(port);
            break;

        case CLEAR_CHECK_TIMER_EXPIRED_EVENT:
            IROS_LOG_MSG(IROS_LOG_LEVEL_MIN, IROS_MID_APP, "CLEAR_TIMER_EXPIRED:port(%d)\n", port);
            loop_clear_check_common_handle(port);
            break;
            
        default:
            IROS_LOG_MSG(IROS_LOG_LEVEL_MIN, IROS_MID_APP, "UNKNOWN EVENT:port(%d)\n", port);
            result = CS_E_PARAM;
            break;
    }
