...
void cs_gpio_cfg_dump(void)
{
    ...
    for(i = 0; i < CS_GPIO_MAX_NUM; i++)
    {
        ...
        case RESET_TYPE:
            cs_printf("%-4d%-8s%-10s%-10s%-13s%-9s%-s\n",i,
                gpio_pin_type_str_get(RESET_TYPE),
                "--","--", "--","--","--");
            break;
        default:
            cs_printf("%-4d%-8s%-10s%-10s%-13s%-9s%s\n",i,
...
static void gpio_polling_timer_handler(void *data)
{
    ...
    for(i = 0; i < CS_GPIO_MAX_NUM; i++)
    {
        ...
        case INPUT_TYPE:
            ...
            {
                /* cs_printf("Please hook polling handler api for GPIO-%d\n",i); */
...
void cs_app_gpio_write(cs_uint8 output_type, cs_uint8 value)
{
    cs_int32 i;
    cs_gpio_pin_t *pPin = NULL;
    cs_callback_context_t context;
    ...
    {
        pPin = (cs_gpio_pin_t *)&gpio_pin[i];
        if(pPin->pin_type != RESET_TYPE)
            continue;

        value = (value)?1:0;
        cs_plat_gpio_write(context, 0, 0, i, value);
        return;
    }
