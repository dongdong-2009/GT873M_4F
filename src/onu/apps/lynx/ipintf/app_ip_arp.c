...
cs_status app_ipintf_arp_keep_alive(void)
{
    ...
    if(PTY_ENABLE)
    	{
    	    ret = app_ipintf_check_ip_address(ipintf_info.ptyip, ipintf_info.ptymask, ipintf_info.ptygtw);
		    if(ret != CS_E_OK) {
		        APP_IPINTF_LOG(IROS_LOG_LEVEL_MIN, " %s, invalid, 0x%8x,0x%8x,0x%8x\n",
    ...
    if (NULL == pkt) {
        APP_IPINTF_LOG(IROS_LOG_LEVEL_CRI, "arp keep alive,no memory\n");
        return -1;
    }
	...
	if(PTY_ENABLE)
		...
		{
			push_vlan_ingress(ipintf_info.uartvlan, ipintf_info.uartpri, pkt);
		}
    /* send packet to aal */
    ret = app_ipintf_send_pkt(CS_PON_PORT_ID, pkt);
    iros_free(pkt);
	PTY_ENABLE = 0;
    APP_IPINTF_LOG(IROS_LOG_LEVEL_DBG3, "%s, send packet, ret %d\n", __func__,ret);
    return ret;
...
cs_status app_ipintf_arp_pre_process(cs_pkt_t* pkt)
{
    ...
    else
    {
     ...
     //   cs_printf(".............................................2\n");
        ...
        if(pkt->port != CS_PON_PORT_ID)  // upstream
        {
            ret = epon_request_onu_port_isolation_get(context, ONU_DEVICEID_FOR_API, ONU_LLIDPORT_FOR_API, &enable);
            APP_IPINTF_LOG(IROS_LOG_LEVEL_DBG3, "Get sport %d isolation mode: %d\n", 
                                                                                      pkt->port, enable);
            if(enable)  // port isolation is enabled.
            {
                APP_IPINTF_LOG(IROS_LOG_LEVEL_DBG3, "RX to TX len %d,sport %d, dport %d\n", 
                                                                                      pkt->len,pkt->port, PTABLE[CS_PON_PORT_ID].port);
                ret |= app_ipintf_send_pkt_with_vlan_check(PTABLE[CS_PON_PORT_ID].port, pkt, packet_buf);
            }
            else  // port isolation is disabled.
            {
        ...
        {
            for(i=1; i<ipintf_info.maxport; i++) {
                if(pkt->port == i) {
                    continue;
                }
