...
#define PTY_IPADDR ipintf_info.ptyip
#define PTY_IPMASK ipintf_info.ptymask
#define PTY_GTW    ipintf_info.ptygtw
...
#define IPINTF_STATIS_INC(type, dport) \
    do{\
        if((dport) >= ipintf_info.maxport) {\
            APP_IPINTF_LOG(IROS_LOG_LEVEL_DBG0, "%s, invalid port %d\n", __func__, (dport));\
            return -1;\
        }\
