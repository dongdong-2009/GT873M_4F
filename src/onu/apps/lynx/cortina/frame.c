...
extern void l2ftp_frame_proc(cs_ether_frame_t *, unsigned short, cs_port_id_t);
...
void onu_frame_proc(cs_ether_frame_t *frame, int len, cs_port_id_t port)
{
	unsigned short ether_type;
	unsigned char *tmp = (unsigned char *)frame;
	unsigned char macaddr[CS_MACADDR_LEN];
    cs_callback_context_t     context;

	if(!frame) return;

	ether_type = *(unsigned short *)(tmp + CS_MACADDR_LEN + CS_MACADDR_LEN);
	ether_type = ntohs(ether_type);

       if (ether_type == EPON_ETHERTYPE_DOT1Q) {

            memcpy(tmp + CS_MACADDR_LEN +CS_MACADDR_LEN,
                   tmp + CS_MACADDR_LEN + CS_MACADDR_LEN + 4,
                   len - CS_MACADDR_LEN -CS_MACADDR_LEN - 4);
   	}
    ...
    epon_request_onu_pon_mac_addr_get(context, ONU_DEVICEID_FOR_API, ONU_LLIDPORT_FOR_API, (cs_mac_t *)macaddr);
        ...
        if (!memcmp(frame->layer3_header.dst.addr, macaddr,
                    CS_MACADDR_LEN) || !memcmp(frame->layer3_header.dst.addr,
                    epon_macaddr_all_onu, CS_MACADDR_LEN)) {
...
{
    if(pPkt->eth_type == CS_ETHERTYPE_IROS) {
...
cs_uint8 iros_pkt_proc(cs_pkt_t *pPkt)
{
    ...
    onu_frame_proc((cs_ether_frame_t *)frame_ptr, len, port);
    return 1;
}
                          ...
                          cs_uint8  mode)
{
    ...
    cs_tagged_ether_frame_t outputFrame;
    cs_ether_frame_t *frame_buf = (cs_ether_frame_t *)frame;
    cs_uint16 vlanId = 0;
    ...
    if( vlanId && port_id != CS_PON_PORT_ID)
    {
        ...
        memset(&outputFrame, 0, sizeof(cs_tagged_ether_frame_t));
        memcpy(&outputFrame.l2dst, &frame_buf->ether_header.dst, sizeof(cs_mac_t));
        memcpy(&outputFrame.l2src, &frame_buf->ether_header.src, sizeof(cs_mac_t));
        ...
        #endif
        memcpy(&outputFrame.layer3_header, &frame_buf->layer3_header, sizeof(cs_layer3_header_t));
...
void setControlPktL2Dest(cs_uint8 *buffer, cs_uint32 len)
{
    ...
    if(!frame || !len || len < CS_FRAME_HEADER_LEN)
            return;
