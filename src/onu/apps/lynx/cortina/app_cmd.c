...
#ifdef HAVE_DB_MANAGMENT
    cs_printf("app db save - Save the db to the flash\n");\
    cs_printf("app db reset- Reset the db to the factory\n");\
#endif
...
#endif
		else if(!strcmp(argv[1], "txpower")){
			 cs_uint8 frame[45] = {0x01, 0x80, 0xc2, 0x00, 0x00, 0x02,
				0x00, 0x13, 0x25, 0x88, 0x00, 0x11, 0x88, 0x09, 0x03,
				0x00, 0x50, 0xfe, 0x11, 0x11, 0x11, 0x03, 0xc7, 0x80, 
				0x1a, 0x0a, 0x00, 0x00, 0x13, 0x25, 0x00, 0x00, 0x01,
				0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
			 cs_pkt_t *pPkt = NULL;

             pPkt = (cs_pkt_t *)iros_malloc(IROS_MID_OAM, sizeof(cs_pkt_t)+45);
             if(!pPkt)
                return SAL_CMD_OK;
             
             memset(pPkt, 0, (sizeof(cs_pkt_t)+45));
			 pPkt->port = 0;
			 pPkt->offset = 0;
			 pPkt->len = 64;
