 ...
 * app_init.c
 *
 * $Id: app_init.c,v 1.1.4.3.2.5.2.4.2.2.2.2.2.3.2.4.2.31.2.58.2.1.2.58.2.1 2011/12/29 07:53:30 fwu Exp $
 */
...
#include "app_oam_siepon.h"
#include "siepon_pwr_saving.h"
#endif
...
#ifdef HAVE_CTC_OAM
    ctc_oam_onu_mc_switch_set_adapt(CTC_MC_SWITCH_START_IMGP_SNOOPING);
#endif
...
void app_task_init()
{
    ...
    onu_8021as_init();
    ptpv2_init();
    onu_ptp_set_mode(PTP_HALF_TC_MODE);
#endif

    app_ref_init();

}
unsigned long long onu_tick,env_tick;
extern cyg_handle_t m0;
extern void onu_event_port_loop(gwd_ethloop_msg_t *msg);
extern void onu_event_port_loop_clear(gwd_ethloop_msg_t *msg);
void app_thread(cyg_addrword_t p)
{
    cs_int32 ret = CS_E_OSAL_OK;
    cs_uint32 real_len = 0;
    iros_msg_t *msg = NULL;
	cs_uint8* loop_msg = NULL;
    cs_uint8  free_msg = 1;
		...
		if(loop_msg)
			{
				switch(*loop_msg)
					{
						case IROS_MSG_GWD_PORT_LOOP_EVENT:
		                    {	
		                    	gwd_ethloop_msg_t *lmsg=(gwd_ethloop_msg_t *)loop_msg;
		                    	if(lmsg->loopstatus == GWD_ETH_PORT_LOOP_ALARM)
		                    	{
		                        	IROS_LOG_INF(26, "recv loop alarm event\r\n");
		                        	onu_event_port_loop(lmsg);
		                    	}
		                    	else
		                    	{
		                        	IROS_LOG_INF(26, "recv loop clear event\r\n");
		                        	onu_event_port_loop_clear(lmsg);
		                    	}
								iros_free(loop_msg);
		                    }
		                      	break;
