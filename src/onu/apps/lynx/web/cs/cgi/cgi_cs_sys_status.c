...
cgi_status cgi_cs_sys_info_get(cgi_handler_param_t * p)
{
    ...
    cs_uint32 i = 0;
#endif
    sys_chip_info_t chip_info;
    sys_onu_info_t  onu_info;
    ...
    memset(&onu_info,0x00,sizeof(onu_info));
    sprintf(buff, "%s", "{");

    cs_plat_chipset_info_get(context,0,0,&chip_info);

    cs_plat_onu_info_get(context,0,0,&onu_info);
    ...
    memset(tmpBuff, 0, sizeof(tmpBuff));
    sprintf(tmpBuff, ",onu_vendor_id:\"%s\"", onu_info.onu_vendor_id);
    strcat(buff, tmpBuff);

    memset(tmpBuff, 0, sizeof(tmpBuff));
    sprintf(tmpBuff, ",onu_model:\"%s\"", onu_info.onu_model);
    strcat(buff, tmpBuff);


    memset(tmpBuff, 0, sizeof(tmpBuff));
    sprintf(tmpBuff, ",onu_hw_ver:\"%s\"",onu_info.hardware_ver);
    strcat(buff, tmpBuff);
    
    memset(tmpBuff, 0, sizeof(tmpBuff));
    sprintf(tmpBuff, ",app_ver:\"%s\"",onu_info.software_ver);
    strcat(buff, tmpBuff);
    
    memset(tmpBuff, 0, sizeof(tmpBuff));
    sprintf(tmpBuff, ",onu_fm_ver:\"%2d.%2d.%2d.%2d\"",
            chip_info.fw_ver.major,
            chip_info.fw_ver.minor,
            chip_info.fw_ver.revision,
            chip_info.fw_ver.build_id);
    strcat(buff, tmpBuff);
    ...
    memset(tmpBuff, 0, sizeof(tmpBuff));
    sprintf(tmpBuff, ",chipset_vendor_id:\"%2x\"",
            chip_info.chip_vendor_id[0]);

    strcat(buff, tmpBuff);


    memset(tmpBuff, 0, sizeof(tmpBuff));
    sprintf(tmpBuff, ",chipset_chip_model:\"%x%x\""
            ,chip_info.chip_model[0]
            ,chip_info.chip_model[1]);
    strcat(buff, tmpBuff);

    memset(tmpBuff, 0, sizeof(tmpBuff));
    sprintf(tmpBuff, ",chipset_rev:\"%x\"",chip_info.rev);
    strcat(buff, tmpBuff);

    memset(tmpBuff, 0, sizeof(tmpBuff));
    sprintf(tmpBuff, ",chipset_ic_ver:\"%s\"",chip_info.date);
    strcat(buff, tmpBuff);
...
cgi_status cgi_cs_ctc_info_get(cgi_handler_param_t * p)
{
    ...
    /* CTC use 1- disable, 2 - enable, we use 0 - disable ,1 - enable*/
    mc_fast_leave--;
